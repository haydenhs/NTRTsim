...
void T6Model::addMarkers(tgStructure &s)
{
    ...
    for(int i=0;i<6;i++)
    {
        const btRigidBody* bt = rods[i]->getPRigidBody();
        btTransform inverseTransform = bt->getWorldTransform().inverse();
        btVector3 pos = inverseTransform * (rods[i]->centerOfMass());
        //btVector3 pos = rods[i]->centerOfMass();
...
void T6Model::setup(tgWorld& world)
{
    ...
    //std::vector<tgBasicActuator*> tempActuators = tgCast::filter<tgModel, tgBasicActuator> (getDescendants());
    //allActuators.assign(tempActuators.begin(),tempActuators.begin()+10);
    // after find all actuators users can employ, what if we choose from here?
    // haydensh's change in 7th,12,2017
